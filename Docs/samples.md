...
2. [Deploy items to a workspace](#4--deploy-items-to-a-workspace)
3. [Promote through a deployment pipeline (Dev → Test → Prod)](#5--promote-through-a-deployment-pipeline)
4. [Multi-stage CI/CD pipeline](#6--multi-stage-cicd-pipeline)
5. [Use a shared deployment script file](#7--use-a-shared-deployment-script-file)
6. [Run on Windows and Linux agents](#8--run-on-windows-and-linux-agents)
...
- A method to extract the `FEDERATED_TOKEN`(OidcToken) from said identity. 
...
## 3 — Promote Through a Deployment Pipeline
...
## 5 — Use a Shared Deployment Script File
...
## 6 — Run on Windows and Linux Agents
